...
def test_csr_mask_sampling():
    ...
    replace = True
    mask = hashmap.query(seeds)
    sub_indptr, sub_indices = capi.csr_mask_sampling(indptr, indices,
                                                     gpu_indptr, gpu_indices,
                                                     seeds, mask, num_picks,
                                                     replace)
    assert torch.equal(
        sub_indptr, torch.tensor([0, 5, 5, 10, 15, 20, 25, 30], device='cuda'))
    assert sub_indices.numel() == 30
...
def benchmark():
    ...
    for size in [10, 100, 1000, 10000, 10_0000, 30_0000, 50_0000]:
        ...
        for j in range(2):
            ...
            with time_recorder("mask", j != 0):
                for i in range(50):
                    mask = hashmap.query(seeds)
                    ## wise sampling
                    sub_indptr, sub_indices = capi.csr_mask_sampling(
                        indptr, indices, gpu_indptr, gpu_indices, seeds, mask,
                        5, False)
